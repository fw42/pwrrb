...
module PwrConnectionHandlerPSK
	def initialize(conn=nil)
		@nonce = 0
		@psk = "X" * 32
		@packer = PwrBSON.new()
	...
	def unbind
		@conn.unbind
		$logger.info("PwrTLS connection with #{@peer[1]}:#{@peer[0]} closed") if @peer
	end
	...
	def connection_completed(*args)
		...
		$logger.info("TCP connection with #{@peer[1]}:#{@peer[0]} established")
	...
	def handle_packet(packet)
		begin
			payload = NaCl.crypto_secretbox_open(packet, nonce_current(), @psk)
			@conn.receive_data(payload)
		rescue NaCl::OpenError
			puts @nonce
			$logger.error("Decryption error!")
...
#		@nonce += 1
	...
	def send(data)
		send_framed(NaCl.crypto_secretbox(data, nonce_next(), @psk))
	end
